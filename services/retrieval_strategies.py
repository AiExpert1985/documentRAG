...
class RetrievalStrategy(ABC):
    """Abstract base class for different retrieval strategies"""
    
    @abstractmethod
    def add_document(self, document_id: str, document_name: str, chunks: List[Any]) -> bool:
        """Add document chunks to the store"""
        pass
    
    @abstractmethod
    def retrieval(self, question: str, top_k: int = 3) -> List[dict]: # Returns a list of dicts with content and metadata
        """retrieval for relevant chunks and return content"""
        pass
        
    @abstractmethod
    def delete_document(self, document_id: str) -> bool:
        """Delete all stored documents for a given document_id"""
        pass
...
class KeywordRetrieval(RetrievalStrategy):
    ...
    def retrieval(self, question: str, top_k: int = 3) -> List[dict]:
...
class HybridRetrieval(RetrievalStrategy):
    ...
    def add_document(self, document_id: str, document_name: str, chunks: List[Any]) -> bool:
        try:
            keyword_success = self.keyword_retrieval.add_document(document_id, document_name, chunks)
            semantic_success = self.semantic_retrieval.add_document(document_id, document_name, chunks)
    ...
    def retrieval(self, question: str, top_k: int = 3) -> List[dict]:
        try:
            keyword_results = self.keyword_retrieval.retrieval(question, min(top_k * 2, 10))
            semantic_results = self.semantic_retrieval.retrieval(question, min(top_k * 2, 10))
            ...
            try:
                return self.semantic_retrieval.retrieval(question, top_k)
    ...
    def delete_document(self, document_id: str) -> bool:
        try:
            keyword_cleared = self.keyword_retrieval.delete_document(document_id)
            semantic_cleared = self.semantic_retrieval.delete_document(document_id)
    ...
    def clear_all_documents(self) -> bool:
        try:
            keyword_cleared = self.keyword_retrieval.clear_all_documents()
            semantic_cleared = self.semantic_retrieval.clear_all_documents()
            success = keyword_cleared and semantic_cleared
            if success:
